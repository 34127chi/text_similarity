...
from bigruatt_model import BiGruAttConfig, BiGruAttModel
...
save_dir = 'model/bigruatt'
model_save_path = os.path.join(save_dir, 'global.encoder.weight')  # 最佳验证结果保存路径
encoder_save_path = os.path.join(save_dir, 'global.model.weight')  # 最佳验证结果保存路径
...
def string2id(s):
    _ = [char2id.get(i, 1) for i in s[:config.seq_len]]#最大长度seq_len  如果不存在的话unknow word 
...
def tune_hyper():
    ...
    dimensions = [hidden_size, activation_func, embedding_dim]
    default_parameters = [128, 'relu', 128]
    ...
    def fitness(hidden_size, activation_func, embedding_dim):
        ...
        if best_acc > best_accuracy:
            model.model.save_weights(global_encoder_save_path)
            model.encoder.save_weights(global_model_save_path)
...
if __name__ == '__main__':
    if len(sys.argv) != 2 or sys.argv[1] not in ['train', 'test', 'tune_hyper']:
        raise ValueError("""usage: python run_bigruatt.py [train / test / tune_hyper]""")

    print('Configuring BiGruAtt model...')
    config = BiGruAttConfig()
