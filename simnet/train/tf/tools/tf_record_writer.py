...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        usage()
        sys.exit(-1)
    input_data_format = sys.argv[1]#输入格式 point-wise pair-wise
    filename = sys.argv[2]#源文件
    tfrecord_name = sys.argv[3]#生成的文件
    pad_id = int(sys.argv[4])#填充的id
    max_len = int(sys.argv[5])#最大长度
    ...
    if input_data_format in data_format_func:
        ...
        sys.exit(1)
    local_writer = tf.python_io.TFRecordWriter(tfrecord_name)#类似文件的引用
