...
def run_trainer(loss, optimizer, config):
    ...
    """
    thread_num = int(config["thread_num"])#线程数
    model_path = config["model_path"]#模型存放路径
    model_file = config["model_prefix"]#模型prefix名称
    print_iter = int(config["print_iter"])#每100次迭代
    data_size = int(config["data_size"])#
    batch_size = int(config["batch_size"])
    epoch_iter = int(data_size / batch_size)
    avg_cost = 0.0
    saver = tf.train.Saver(max_to_keep=None)#保留最大吗
    init = tf.group(tf.global_variables_initializer(),
                    tf.local_variables_initializer())
    with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=thread_num, 
                                          inter_op_parallelism_threads=thread_num)) \
                    as sess:#运算 之间与内部的并行运算
        sess.run(init)#初始化参数
        coord = tf.train.Coordinator()#线程协调用
        read_thread = tf.train.start_queue_runners(sess=sess, coord=coord)
        step = 0
        epoch_num = 1
