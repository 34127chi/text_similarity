...
class CNNDynamicPoolingLayer(object):
    ...
    def ops(self, cross, mask=None):
        ...
        conv = tf.nn.conv2d(emb_expanded, self.conv_w,
                            ...
                            padding="SAME",
                            name="conv_op")
        h = tf.nn.bias_add(conv, self.bias)#batch_size seq_len embedding num_filters
        if mask is not None:#剔除掉那些填充的部分
        ...
        pool_flat = tf.reshape(
            pool, [-1, self.dpool_size1 * self.dpool_size2 * self.kernel_size])#batch_size self.dpool_size1 * self.dpool_size2 * self.kernel_size
        return pool_flat
...
class SimilarityMatrixLayer(object):
    ...
    def ops(self, query_emb, title_emb):
        ...
        query_norm = tf.sqrt(tf.reduce_sum(
            tf.square(query_emb), 2, keep_dims=True))#batch_size seq_len emb_size
        normalized_query_emb = query_emb / query_norm#归一化处理
        title_norm = tf.sqrt(tf.reduce_sum(
            tf.square(title_emb), 2, keep_dims=True))
        normalized_title_emb = title_emb / title_norm#归一化处理
        sim_mat = tf.matmul(normalized_query_emb, tf.transpose(
            normalized_title_emb, perm=[0, 2, 1]), name="similarity_matrix")#batch_size seq_len seq_len
        return sim_mat
