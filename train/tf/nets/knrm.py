...
class KNRM(object):
    ...
    流程:input1 embedding + input2 embedding => input1 与 input2 相似度矩阵(归一化后的) => kenerl池化层(多种尺度的池化层 池化层的泛化 高斯分布)  => 全连接层
    """
    def __init__(self, config):
        ...
        self.sim_mat_layer = layers.SimilarityMatrixLayer()#相似矩阵
        self.kernel_pool_layer = layers.KernelPoolingLayer(self.kernel_num, self.lamb)
        self.tanh_layer = layers.TanhLayer()#tanh激活函数
