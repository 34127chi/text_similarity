...
class MLPCnn(object):
    ...
    流程:input1 embedding + input2 embedding => input1 encode(birnn) + input2 encode(birnn) => input1 与 input2 相似度矩阵 => 卷积+池化层  => 全连接层
    """
    def __init__(self, config):
        ...
        self.emb_layer = layers.EmbeddingLayer(self.vocab_size, self.emb_size)#embedding矩阵
        self.cnn_layer = layers.CNNLayer(self.seq_len, self.emb_size, 
                                        self.win_size, self.kernel_size)#卷积池化层
        ...
        if self.task_mode == "pointwise":
            ...
            self.fc1_layer = layers.FCLayer(2 * self.kernel_size, self.hidden_size)#全连接层1 2*self.kernel_size hidden_size
            self.fc2_layer = layers.FCLayer(self.hidden_size, self.n_class)#全连接层2 hidden_size * n_class
